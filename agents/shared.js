...
export function assertZhipuApiKey() {
  if (!config.zhipu.apiKey) {
    throw new Error('请配置 ZHIPU_API_KEY 环境变量（在 .env 文件中设置）');
  }
}

export function createZhipuLlm({ temperature = 0.3, maxTokens = 4096 } = {}) {
  return new ChatOpenAI({
    modelName: config.zhipu.model,
    openAIApiKey: config.zhipu.apiKey,
    configuration: {
